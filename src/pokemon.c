...
s32 CalculateBaseDamage(struct BattlePokemon *attacker, struct BattlePokemon *defender, u32 move, u16 sideStatus, u16 powerOverride, u8 typeOverride, u8 battlerIdAtk, u8 battlerIdDef)
{
    ...
    else //special
    {
        ...
        if (gStatuses3[battlerIdAtk] & STATUS3_CHARGED_UP && gBattleMoves[gCurrentMove].type == TYPE_ELECTRIC && gBattleMoves[gCurrentMove].power > 0)
        gBattleMoveDamage *= 2;
        gStatuses3[battlerIdAtk] &= ~STATUS3_CHARGED_UP;
    ...
    // Moves hitting multiple targets do 3/4 damage in double battles
    if ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE) && (gBattleMoves[move].target == (MOVE_TARGET_FOES_AND_ALLY || MOVE_TARGET_BOTH)))
    {
        if ((gBattleMoves[move].target == MOVE_TARGET_BOTH && CountAliveMonsInBattle(BATTLE_ALIVE_DEF_SIDE) == 2)
        || (gBattleMoves[move].target == MOVE_TARGET_FOES_AND_ALLY && CountAliveMonsInBattle(BATTLE_ALIVE_EXCEPT_ACTIVE) > 1))
            damage = (3 * damage) / 4;
    }
