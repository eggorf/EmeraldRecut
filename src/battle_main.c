...
EWRAM_DATA u16 gBattleWeather = B_WEATHER_NONE;
...
static void TryDoEventsBeforeFirstTurn(void)
{
    ...
    {
        BattlerByOrder = gBattlerByTurnOrder[gBattleStruct->switchInAbilitiesCounter];
       
        if (AbilityBattleEffects(ABILITYEFFECT_ON_SWITCHIN, BattlerByOrder, 0, 0, 0) != 0) //if switch in ability
            effect++;
        if ((AbilityBattleEffects(ABILITYEFFECT_IMMUNITY, BattlerByOrder, 0, 0, 0) != 0) //immunity and currently statused
            && ((gBattleMons[BattlerByOrder].status1 & STATUS1_ANY)
                || gBattleMons[BattlerByOrder].status2 & (STATUS2_CONFUSION || STATUS2_INFATUATION)))
        {
            effect++;
        }
...
static void ReturnFromBattleToOverworld(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_ROAMER)
    {
        ...
        if (((gBattleOutcome == B_OUTCOME_WON) || gBattleOutcome == B_OUTCOME_CAUGHT) && gBattleOutcome != B_OUTCOME_PLAYER_TELEPORTED)
            SetRoamerInactive();
    }
