...
//static void Cmd_JumpIfNoAlly(void);
...
static void Cmd_accuracycheck(void)
{
    ...
    else
    {
        ...
        if (gBattleMons[gBattlerTarget].statStages[STAT_EVASION] > DEFAULT_STAT_STAGE)
        {
            if (gBattleMons[gBattlerTarget].status2 & STATUS2_FORESIGHT || (gBattleMons[gBattlerAttacker].ability == (ABILITY_KEEN_EYE || ABILITY_ILLUMINATE)))
            {
...
// This is ModulateDmgByType2, instances of MDBT1 were rewritten to plug into it
static void ModulateDmgByType(u8 multiplier, u16 move, u8 *flags) //MDBT1 uses same multiplier, gCurrentMove, gMoveResultFlags
{
...
static void Cmd_adjustnormaldamage(void)
{
    u8 holdEffect, param, Endured, HungOn;
    ...
    else if (gBattleMons[gBattlerTarget].ability == ABILITY_STURDY && gBattleMons[gBattlerTarget].hp == gBattleMons[gBattlerTarget].maxHP)
    {
...
static void Cmd_resultmessage(void)
{
    ...
    else
    {
        ...
        case MOVE_RESULT_DOESNT_AFFECT_FOE:
            ...
            break;
        case MOVE_RESULT_FOE_HUNG_ON:
            if (gBattleMons[gBattlerTarget].item == ITEM_FOCUS_BAND)
            {
                gLastUsedItem = gBattleMons[gBattlerTarget].item;
                gPotentialItemEffectBattler = gBattlerTarget;
            ...
            else
            {
                ...
                gMoveResultFlags &= ~(MOVE_RESULT_FOE_ENDURED | MOVE_RESULT_FOE_HUNG_ON);
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_SturdyBandActivates;
                return;
            }
        default:
            if (gMoveResultFlags & MOVE_RESULT_DOESNT_AFFECT_FOE)
    ...
    if (gBattleCommunication[MOVE_EFFECT_BYTE] <= PRIMARY_STATUS_MOVE_EFFECT)
    {
        ...
        if (statusChanged == TRUE)
        {
            ...
            if (sStatusFlagsForMoveEffects[gBattleCommunication[MOVE_EFFECT_BYTE]] == STATUS1_SLEEP)
                gBattleMons[gEffectBattler].status1 |= STATUS1_SLEEP_TURN((Random() & 2) + 2); // 2-4 turns
            else
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_KNOCK_OFF:
                ...
                else if (gBattleMons[gEffectBattler].ability == ABILITY_STICKY_HOLD)
                {
    ...
    case 1: // calculate experience points to redistribute
        {
			...
			//* ((2*gBattleMons[gBattlerFainted].level+10)/(gBattleMons[gBattlerFainted].level+GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL)+10)) needs exponent
        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP))
                {
                    ...
                    if (IsTradedMon(&gPlayerParty[gBattleStruct->expGetterMonId])) //if mon is traded
                    {
                        gBattleMoveDamage = (gBattleMoveDamage * 150) / 100;
                        i = STRINGID_ABOOSTED;
                    }
...
static void Cmd_playanimation(void)
{
    ...
    else if (gHitMarker & HITMARKER_NO_ANIMATIONS)
    {
        ...
        gBattlescriptCurrInstr = BattleScript_Pausex20;
          ...
          || gBattlescriptCurrInstr[2] == B_ANIM_SNOW_CONTINUES)
    {
        BtlController_EmitBattleAnimation(BUFFER_A, gBattlescriptCurrInstr[2], *argumentPtr);
        MarkBattlerForControllerExec(gActiveBattler);
        gBattlescriptCurrInstr += 7;
...
static void Cmd_playanimation_var(void)
{
    ...
    else if (gHitMarker & HITMARKER_NO_ANIMATIONS)
    {
        gBattlescriptCurrInstr += 10;
          ...
          || *animationIdPtr == B_ANIM_SNOW_CONTINUES)
    {
        BtlController_EmitBattleAnimation(BUFFER_A, *animationIdPtr, *argumentPtr);
        MarkBattlerForControllerExec(gActiveBattler);
        gBattlescriptCurrInstr += 10;
...
static void Cmd_playstatchangeanimation(void)
{
    ...
    if (gBattlescriptCurrInstr[3] & STAT_CHANGE_NEGATIVE) // goes down
    {
        ...
        while (statsToCheck != 0)
        {
            if (statsToCheck & 1)
            {
                if ((gBattlescriptCurrInstr[3] & STAT_CHANGE_CANT_PREVENT) && gBattleMons[gActiveBattler].statStages[currStat] > MIN_STAT_STAGE) 
                {
                    statAnimId = startingStatAnimId + currStat;
                    changeableStatsCount++;
                }
                else if (!gSideTimers[GET_BATTLER_SIDE(gActiveBattler)].mistTimer
                        && gBattleMons[gActiveBattler].ability != (ABILITY_CLEAR_BODY || ABILITY_WHITE_SMOKE)
                        && !(gBattleMons[gActiveBattler].ability == (ABILITY_KEEN_EYE || ABILITY_ILLUMINATE) && currStat == STAT_ACC)
...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_ON_DAMAGE_ABILITIES: // Such as abilities activating on contact (Effect Spore, Rough Skin, etc.).
            ...
            gBattleScripting.moveendState++;
            break;
        case MOVEEND_ON_DAMAGE_ATTACKER: // Poison Touch, Stench, etc
...
static u8 ChangeStatBuffs(s8 statValue, u8 statId, u8 flags, const u8 *BS_ptr)
{
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        else if (gBattleMons[gActiveBattler].ability == (ABILITY_KEEN_EYE || ABILITY_ILLUMINATE)
...
static void Cmd_setmultihitcounter(void)
{
    ...
    {
        gMultiHitCounter = (Random() & 3) + 2; //25% each
        if (gMultiHitCounter > 3 && ((Random() & 9) > 3)) //uppers have 40% to reroll
            gMultiHitCounter = (Random() & 3) + 2; //new 25% from 40% of uppers, equivalent to +10% lowers
    }
...
{
    if ((gBattleMons[gBattlerAttacker].level >= gBattleMons[gBattlerTarget].level) || (gBattleTypeFlags & BATTLE_TYPE_TRAINER))
    {
...
static void Cmd_tryKO(void) //ohko's
{
    ...
    if (gChosenMove == MOVE_SHEER_COLD && IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_ICE))
    {
        gMoveResultFlags |= MOVE_RESULT_MISSED;
    ...
    if (gBattleMons[gBattlerTarget].ability == ABILITY_STURDY) //THE ACTUAL OHKO SCRIPT
    {
        gMoveResultFlags |= MOVE_RESULT_MISSED;
        gLastUsedAbility = ABILITY_STURDY;
...
static void Cmd_trydobeatup(void)
{
    ...
    else
    {
        ...
        gBattleMoveDamage = gBattleMons[gBattlerAttacker].attack;
        gBattleMoveDamage *= (gBattleMons[gBattlerAttacker].level * 2 / 5 + 2);

        for (;gBattleCommunication[0] < PARTY_SIZE; gBattleCommunication[0]++)
        {
            if (GetMonData(&party[gBattleCommunication[0]], MON_DATA_HP)
                && GetMonData(&party[gBattleCommunication[0]], MON_DATA_SPECIES_OR_EGG) != (SPECIES_NONE | SPECIES_EGG)
        ...
        if (gBattleCommunication[0] < PARTY_SIZE)
        {
            ...
            gBattleMoveDamage *= (gSpeciesInfo[GetMonData(&party[gBattleCommunication[0]], MON_DATA_SPECIES)].baseAttack / 10 + 5);
            gBattleMoveDamage /= gBattleMons[gBattlerTarget].defense;
            gBattleMoveDamage = (gBattleMoveDamage / 50) + 2;
            if (gProtectStructs[gBattlerAttacker].helpingHand)
                gBattleMoveDamage = gBattleMoveDamage * 150 / 100;

            gMoveResultFlags = TypeCalc(MOVE_BEAT_UP, gBattlerAttacker, gBattlerTarget);
...
static void Cmd_trymemento(void)
{
    if ((gBattleMons[gBattlerTarget].statStages[STAT_ATK] == MIN_STAT_STAGE
        ...
        || gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
    {
        // Failed, protected, or already has minimum Attack and Special Attack.
        gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
    }
    else
    {
...
static void Cmd_settaunt(void) //3 turns if taunt is first, 4 if taunt is 2nd.
{
...
/* HOWL
void Cmd_JumpIfNoAlly(void)
{
    u32 partner = BATTLE_PARTNER(GetBattlerForBattleScript(gActiveBattler));
